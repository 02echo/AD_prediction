...
from model import CAT_clinical_inception
...
def main(options):
    # Path configuration
    TXT_ALL_PATH = "./data/y_rfe.txt"  # y_rfe  ref_all_gene.txt

    IMAGE_PATH = "./data/wm_image"

    TXT_TEST_PATH = "./data/y_ref_test.txt"
    ...
    for train_index, test_index in kf.split(x):  # train_index是数组下标索引
        ...
        model = CAT_clinical_inception.mobilevit_s_fusion()
        ...
        train_data = AD_2DSlicesDataFusion(train_index, IMAGE_PATH, TXT_ALL_PATH)
        ...
        for epoch_i in range(options.epochs):
            ...
            for it, train_data in enumerate(train_loader):
                ...
                for data_dic in train_data:
                    ...
                    train_output, train_feature = model(imgs, clinical)
                    ...
                    #支持向量机SVM
                    svm_classifier = SVC(kernel='poly', C=1, gamma=1, decision_function_shape='ovr')
        ...
        # test
        # model = load_checkpoint(model.state_dict(), model, 'cuda')
        model.eval()
        with torch.no_grad():
            for it, test_data in enumerate(valid_loader):  #test_loader
                ...
                for data_dic in test_data:
                    ...
                    count_test += 1
                    test_output, test_feature = model(imgs, clinical)#, gene)
                    ...
                    recall_knn = recall_score(test_labels, y_pred_knn, average='micro')  # 三类别加average='micro'
