...
from model import concat_MobileVit_change_inception
...
def main(options):
    # Path configuration
    TXT_ALL_PATH = "D:\\graduate\\project\\DATA\\coregister-1000\\ref_all_gene.txt"
    ...
    for train_index, test_index in kf.split(x):  # train_index是数组下标索引
        ...
        model = concat_MobileVit_change_inception.mobilevit_s()
        ...
        for epoch_i in range(options.epochs):
            ...
            for it, train_data in enumerate(train_loader):
                ...
                for data_dic in train_data:
                    ...
                    train_feature, train_output = model(imgs, clinical, gene)
                    ...
                    # KNN train
                    knn_classifier = KNeighborsClassifier(n_neighbors=16)  # p:距离计算方式 , p=1
                    ...
                    #测试最佳 c:0.1 gamma:0.01 kernel:linear
                    # if epoch_i == options.epochs - 1:
                    #     svm_classifier = GridSearchCV(SVC(), param_grid={"C": [0.1, 1, 10], "gamma": [0.01, 0.1, 1], "kernel": ['linear', 'poly', 'rbf', 'sigmoid']},
                    #                                   cv=4)
                    #     svm_classifier.fit(train_feature, labels)
        ...
        with torch.no_grad():
            for it, test_data in enumerate(valid_loader):
                ...
                for data_dic in test_data:
                    ...
                    count_test += 1
                    test_feature, test_output = model(imgs, clinical, gene)
        ...
        precision = [0] * 3  # TP/TP+FP  预测为正的里面有几个是正确的
        recall = [0] * 3  # TP/TP+FN  全部的正例里面有recall%个预测对了
        ...
        logging.info(
            "Val Loss:{0:.5f}..Val Accuracy:{1:.6f}..".format(
                test_loss, test_acc))
        logging.info(
...
def train(i_epoch, model, train_loader, test_loader, use_cuda, criterion, optimizer, scheduler, batch_size, device="cuda"):
    # main training loop
    train_loss = 0.0
    train_correct_sum = 0
    train_simple_cnt = 0
    ...
    for it, train_data in enumerate(train_loader):
        ...
        for data_dic in train_data:
            imgs, labels, clinical, gene = data_dic['image'].to(device), data_dic['label'].to(device), data_dic['clinical'].to(device), data_dic['gene'].to(device)
    ...
    with torch.no_grad():
        for it, test_data in enumerate(test_loader):
            ...
            for data_dic in test_data:
                ...
                imgs, test_labels, clinical, gene = data_dic['image'].to(device), data_dic['label'].to(device), data_dic['clinical'].to(device), data_dic['gene'].to(device)
                count_test += 1
    ...
    return train_acc, train_loss, test_acc, test_loss, precision, recall, F1
